...
#| fig-align: center
#| fig-cap: 
#|   - "plot of tooth length by two supplement delivery methods"
#|   - "plot of tooth length by three dosages uses"
#| cap-align: center
...
## One Sample t-test w/ Analysis 

```{r}
t.test(ToothGrowth$len)
```
### Conclusion/Analysis
(@) I can conlude that there is a difference between *the treatment mean tooth length for the OJ supplement delivery method and the treatment mean tooth length for the VC supplement delivery method* based on the p-value of 2.2e-16 which is significanly smaller than the .05 benchmark for significance.
(@) I am 95% confident that the long term true difference of averages between *the OJ supplement delivery method* **and** *the treatment mean tooth length for the VC supplement delivery method* lies between 16.83731 and 20.78936.
